...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching.Events
{
    ...
    public class TournamentInfoCI : SportEventCI, ITournamentInfoCI
    {
        ...
        {
            if (_tournamentCoverage != null) return _tournamentCoverage;
            await FetchMissingSummary(new[] {DefaultCulture}, false).ConfigureAwait(false);
        ...
        public async Task<IEnumerable<CompetitorCI>> GetCompetitorsAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            wantedCultures = LanguageHelper
                .GetMissingCultures(wantedCultures, _competitors?.FirstOrDefault()?.Names.Keys).ToList();
            if (_competitors != null && !wantedCultures.Any()) return _competitors;
        ...
        {
            var wantedCultures = cultures as CultureInfo[] ?? cultures.ToArray();
            if (_groups != null && !LanguageHelper.GetMissingCultures(wantedCultures,
                    _groups.FirstOrDefault()?.Competitors?.FirstOrDefault()?.Names.Keys).Any()) return _groups;
            await FetchMissingSummary(wantedCultures, false).ConfigureAwait(false);
            return _groups;
        }
        ...
        public async Task<IEnumerable<URN>> GetScheduleAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                var tasks = missingCultures.Select(s => DataRouterManager.GetSportEventsForTournamentAsync(Id, s, this))
                    .ToList();
                ...
                if (tasks.All(a => a.IsCompleted))
                {
                    ...
                    if (tasks.First().Result != null) _scheduleUrns = tasks.First().Result.Select(s => s.Item1);
                }
            }
        ...
        {
            var wantedCultures = cultures as CultureInfo[] ?? cultures.ToArray();
            if (_round != null && _round.HasTranslationsFor(wantedCultures)) return _round;
            await FetchMissingSummary(wantedCultures, false).ConfigureAwait(false);
            return _round;
        }
        ...
        {
            if (_tournamentInfoBasic != null) return _tournamentInfoBasic;
            await FetchMissingSummary(new[] {DefaultCulture}, false).ConfigureAwait(false);
        ...
        {
            if (_referenceId != null) return _referenceId;
            await FetchMissingSummary(new[] {DefaultCulture}, false).ConfigureAwait(false);
            return _referenceId;
        }
        ...
        public async Task<IEnumerable<URN>> GetSeasonsAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                var tasks = missingCultures.Select(s => DataRouterManager.GetSeasonsForTournamentAsync(Id, s, this))
                    .ToList();
                await Task.WhenAll(tasks).ConfigureAwait(false);
        ...
        {
            if (_competitorsReferences != null) return _competitorsReferences;
            await FetchMissingSummary(new[] {DefaultCulture}, false).ConfigureAwait(false);
        ...
        private void ActualMerge(TournamentInfoDTO dto, CultureInfo culture)
        {
            ...
            if (dto.Category != null) _categoryId = dto.Category.Id;
            if (dto.TournamentCoverage != null) _tournamentCoverage = new TournamentCoverageCI(dto.TournamentCoverage);
            if (dto.Competitors != null)
            {
                if (_competitors == null)
                    _competitors =
                        new List<CompetitorCI>(dto.Competitors.Select(t =>
                            new CompetitorCI(t, culture, DataRouterManager)));
                else
            ...
            if (dto.Groups != null)
            {
                ...
                foreach (var groupDTO in dto.Groups) comps.AddRange(groupDTO.Competitors);
            ...
            if (!string.IsNullOrEmpty(dto.Year)) _year = dto.Year;
            ...
            if (dto.SeasonCoverage != null) _seasonCoverage = new SeasonCoverageCI(dto.SeasonCoverage);
        }
        ...
        private void ActualMergeFixture(FixtureDTO fixture, CultureInfo culture)
        {
            ...
            if (fixture.ReferenceIds != null) _referenceId = new ReferenceIdCI(fixture.ReferenceIds);
        }
        ...
        {
            if (competitors == null) return;
            if (_competitorsReferences == null) _competitorsReferences = new Dictionary<URN, ReferenceIdCI>();
            foreach (var competitor in competitors)
