...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching.Events
{
    ...
    public class SportEventCI : ISportEventCI
    {
        ...
        /// <summary>
        ///     A <see cref="CultureInfo" /> specifying the language used when fetching info which is not translatable (e.g.
        ///     Scheduled, ..)
        /// </summary>
        ...
        /// <summary>
        ///     A <see cref="MemoryCache" /> used to cache the sport events fixture timestamps
        /// </summary>
        protected readonly MemoryCache FixtureTimestampCache;
        ...
        /// <summary>
        ///     A <see cref="ISemaphorePool" /> instance used to obtain <see cref="SemaphoreSlim" /> instances used to sync async
        ///     calls
        /// </summary>
        ...
        public async Task<IReadOnlyDictionary<CultureInfo, string>> GetNamesAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            if (HasTranslationsFor(cultureInfos)) return Names;
        ...
        {
            if (_sportId != null) return _sportId;
            if (LoadedSummaries.Any()) return _sportId;
        ...
        /// </summary>
        /// <returns>
        ///     A <see cref="DateTime" /> instance specifying when the sport event associated with the current
        ///     instance was scheduled, or a null reference if the time is not known
        /// </returns>
        public async Task<DateTime?> GetScheduledAsync()
        {
            if (_scheduled != null && _scheduled > DateTime.MinValue) return _scheduled;
            if (LoadedSummaries.Any()) return _scheduled;
            await FetchMissingSummary(new List<CultureInfo> {DefaultCulture}, false).ConfigureAwait(false);
            return _scheduled;
        }
        ...
        {
            if (_scheduledEnd != null && _scheduledEnd > DateTime.MinValue) return _scheduledEnd;
            if (_scheduled != null && _scheduled > DateTime.MinValue
            ) // if _schedule is loaded, then something was prefetched
                return _scheduledEnd;
            if (LoadedSummaries.Any()) return _scheduledEnd;
            await FetchMissingSummary(new List<CultureInfo> {DefaultCulture}, false).ConfigureAwait(false);
            return _scheduledEnd;
        }

        /// <summary>
        ///     Asynchronously gets a <see cref="Nullable{bool}" /> specifying if the start time to be determined is set for the
        ///     associated sport event.
        /// </summary>
        /// <returns>
        ///     A <see cref="Nullable{bool}" /> specifying if the start time to be determined is set for the associated sport
        ///     event.
        /// </returns>
        public async Task<bool?> GetStartTimeTbdAsync()
        {
            if (_startTimeTbd != null) return _startTimeTbd;
            if (LoadedSummaries.Any()) return _startTimeTbd;
            await FetchMissingSummary(new List<CultureInfo> {DefaultCulture}, false).ConfigureAwait(false);
        ...
        {
            if (_replacedBy != null) return _replacedBy;
            if (LoadedSummaries.Any()) return _replacedBy;
            await FetchMissingSummary(new List<CultureInfo> {DefaultCulture}, false).ConfigureAwait(false);
            return _replacedBy;
        }
        ...
        protected async Task FetchMissingSummary(IEnumerable<CultureInfo> cultures, bool forceFetch)
        {
            ...
            if (!missingCultures.Any() && !forceFetch) return;
            ...
            try
            {
                ...
                missingCultures = forceFetch
                    ? cultureInfos.ToList()
                    : LanguageHelper.GetMissingCultures(cultureInfos, LoadedSummaries).ToList();
                if (!missingCultures.Any()) return;
            ...
            catch (CommunicationException ce)
            {
                if (ce.Message.Contains("NotFound")) // especially for tournaments that dont have summary
                {
                    ...
                    ExecutionLog.Warn(
                        $"Fetching summary for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH NOT_FOUND.");
                }
                else
                {
                    ExecutionLog.Warn(
                        $"Fetching summary for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH ERROR.");
                    if (((DataRouterManager) DataRouterManager).ExceptionHandlingStrategy ==
                        ExceptionHandlingStrategy.THROW) throw;
                }
            }
            catch (Exception ex)
            {
                ExecutionLog.Error(
                    $"Fetching summary for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH EX.",
                    ex);
        ...
        protected async Task FetchMissingFixtures(IEnumerable<CultureInfo> cultures)
        {
            ...
            catch (CommunicationException ce)
            {
                if (ce.Message.Contains("NotFound"))
                {
                    LoadedFixtures.AddRange(missingCultures);
                    ExecutionLog.Warn(
                        $"Fetching fixtures for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH NOT_FOUND.");
                }
                else
                {
                    ExecutionLog.Warn(
                        $"Fetching fixtures for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH ERROR.");
                    if (((DataRouterManager) DataRouterManager).ExceptionHandlingStrategy ==
                        ExceptionHandlingStrategy.THROW) throw;
                }
            }
            ...
            {
                ExecutionLog.Error(
                    $"Fetching fixtures for eventId={Id} for languages [{string.Join(",", missingCultures)}] COMPLETED WITH EX.",
                    ex);
                if (((DataRouterManager) DataRouterManager).ExceptionHandlingStrategy ==
                    ExceptionHandlingStrategy.THROW) throw;
            }
