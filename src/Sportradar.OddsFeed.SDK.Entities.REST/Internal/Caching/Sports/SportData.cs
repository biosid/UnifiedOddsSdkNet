...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching.Sports
{
    ...
    {
        /// <summary>
        ///     Gets the <see cref="IEnumerable{T}" /> representing the categories, which belong to the sport
        ...
        ///     the <see cref="IEnumerable{CategoryData}" /> representing the categories, which belong to the
        ///     sport
        /// </param>
        ...
        {
            if (categories != null)
                Categories = categories as ReadOnlyCollection<CategoryData> ??
                             new ReadOnlyCollection<CategoryData>(categories.ToList());
        }
    }
}