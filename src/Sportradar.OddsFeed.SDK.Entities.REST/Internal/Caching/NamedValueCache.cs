...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class NamedValueCache : INamedValueCache
    {
        ...
        /// </param>
        public NamedValueCache(IDataProvider<EntityList<NamedValueDTO>> dataProvider,
        ...
        public bool IsValueDefined(int id)
        {
            lock (_lock)
            {
                if (!_dataFetched) _dataFetched = FetchAndMerge();
            }
        ...
        public INamedValue GetNamedValue(int id)
        {
            ...
            string description;
            if (_namedValues.TryGetValue(id, out description)) return new NamedValue(id, description);
        ...
        private bool FetchAndMerge()
        {
            ...
            foreach (var item in record.Items) _namedValues[item.Id] = item.Description;
