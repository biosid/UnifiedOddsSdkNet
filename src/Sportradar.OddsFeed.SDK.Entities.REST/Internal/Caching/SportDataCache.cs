...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class SportDataCache : SdkCache, ISportDataCache, IDisposable, IHealthStatusProvider
    {
        ...
        /// <summary>
        ///     A <see cref="IEnumerable{T}" /> specifying cultures in which the sport data is periodically fetched
        /// </summary>
            ...
            : base(cacheManager)
        {
            ...
            _requiredCultures = cultures as ReadOnlyCollection<CultureInfo> ??
                                new ReadOnlyCollection<CultureInfo>(cultures.ToList());
        ...
        {
            HealthChecks.RegisterHealthCheck("SportDataCache", StartHealthCheck);
        }
        ...
        public HealthCheckResult StartHealthCheck()
        {
            return Sports.Any()
                ? HealthCheckResult.Healthy(
                    $"Sports has {Sports.Count} items, Categories has {Categories.Count} items.")
                : HealthCheckResult.Unhealthy("Dictionary is empty.");
        }

        /// <summary>
        ///     Asynchronously gets a <see cref="IEnumerable{SportData}" /> representing sport hierarchies for all sports supported
        ///     by the feed.
        /// </summary>
        ...
        public async Task<IEnumerable<SportData>> GetSportsAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            if (!await _semaphore.WaitAsyncSafe().ConfigureAwait(false)) return null;
            ...
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sports data for: {string.Join(",", missingCultures)}. Exception: {ex}");
                throw;
            }
            finally
            {
        ...
        public async Task<SportData> GetSportAsync(URN id, IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                var missingCultures = cultureList.Where(c => !FetchedCultures.Contains(c)).ToList();
                if (missingCultures.Any()) await FetchAndMergeAll(missingCultures, false).ConfigureAwait(false);
                return await GetSportFromCacheAsync(id, cultureList).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for: id={id}, cultures={string.Join(",", cultureList)}. Exception: {ex}");
                throw;
            }
            finally
            {
        ...
        public async Task<CategoryData> GetCategoryAsync(URN id, IEnumerable<CultureInfo> cultures)
        {
            ...
            try
            {
                if (Categories.TryGetValue(id, out categoryCI))
                {
                    if (categoryCI.HasTranslationsFor(cultureList))
                        return new CategoryData(id,
                            categoryCI.Name.Where(t => cultureList.Contains(t.Key))
                                .ToDictionary(kvp => kvp.Key, kvp => kvp.Value), categoryCI.CountryCode,
                            categoryCI.TournamentIds);
            ...
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch category data for: id={id}, cultures={string.Join(",", cultureList)}. Exception: {ex}");
            }
            finally
            {
                _semaphore.ReleaseSafe();
        ...
        /// <summary>
        ///     Asynchronously gets a <see cref="SportData" /> representing sport associated with the tournament specified by it's
        ///     id. Note that the hierarchy will only contain the
        ///     specified tournament and it's parent category not all categories / tournaments in the hierarchy
        /// </summary>
        /// <param name="tournamentId">
        ///     A <see cref="URN" /> specifying the id of the tournament whose parent sport should be
        ///     retrieved
        /// </param>
        /// <param name="cultures">A <see cref="IEnumerable{CultureInfo}" /> specifying the languages in which the data is returned</param>
        /// <returns>A <see cref="Task{SportData}" /> representing the asynchronous operation</returns>
        public async Task<SportData> GetSportForTournamentAsync(URN tournamentId, IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for tournament: id={tournamentId}, cultures={string.Join(",", cultureList)}. Exception: {ex}");
                return null;
            }
            finally
            {
        ...
        /// <summary>
        ///     Fetches the data for pre-configured languages and merges it to internally used dictionaries. First time the method
        ///     is invoked it fetches the data only for missing languages.
        ///     On subsequent calls data for all configured languages is fetched and dictionary are cleared before fetched data is
        ///     added / merged
        /// </summary>
        ...
        {
            if (!await _semaphore.WaitAsyncSafe().ConfigureAwait(false)) return;
            ...
            try
            {
                ...
                if (cultureInfos.Any())
                {
                    ...
                    ExecutionLog.Info(
                        $"Sport data for languages [{string.Join(",", cultureInfos)}] successfully fetched and merged.");
            ...
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for: {string.Join(",", cultureInfos)}. Exception was: {ex}");
            }
            catch (ObjectDisposedException)
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for: {string.Join(",", cultureInfos)}. DataProvider was already disposed.");
            }
            catch (TaskCanceledException)
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for: {string.Join(",", cultureInfos)}. Task canceled. DataProvider was already disposed.");
            }
            catch (Exception ex)
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to fetch sport data for: {string.Join(",", cultureInfos)}. Exception: {ex}");
            }
            finally
            {
                _semaphore.ReleaseSafe();
        ...
        /// <summary>
        ///     Fetches the data for languages specified by <code>cultures</code> and merges / adds it to internally used
        ///     dictionaries.
        /// </summary>
        /// <param name="cultures">A <see cref="IEnumerable{CultureInfo}" /> specifying the languages in which to fetch the data</param>
        /// <param name="clearExistingData">
        ///     Value indicating whether the internally used dictionaries should be cleared before
        ///     fetched data is added
        /// </param>
        ...
        private async Task FetchAndMergeAll(IEnumerable<CultureInfo> cultures, bool clearExistingData)
        {
            ...
            Metric.Context("CACHE").Meter("SportDataCache->FetchAndMergeAll", Unit.Calls).Mark(
                $"Getting for cultures='{string.Join(",", cultureInfos.Select(c => c.TwoLetterISOLanguageName))}'.");

            var fetchTasks = cultureInfos.Select(c => _dataRouterManager.GetAllSportsAsync(c)).ToList();
            fetchTasks.AddRange(cultureInfos.Select(c => _dataRouterManager.GetAllTournamentsForAllSportAsync(c))
                .ToList());
            ...
            foreach (var culture in cultureInfos) FetchedCultures.Add(culture);
        }

        /// <summary>
        ///     Gets a <see cref="SportData" /> representing the parent sport of the tournament specified by
        ///     <code>tournamentId</code> in the languages specified by <code>cultures</code>, or a null reference
        ///     if the specified sport does not exist, or it(or one of it's children) is not available in one of the requested
        ///     languages
        /// </summary>
        /// <remarks>
        ///     The returned <see cref="SportData" /> represents a sport with flattened hierarchy information - only one category
        ///     and one tournament are found in the returned instance.
        /// </remarks>
        /// <param name="tournamentId">A <see cref="URN" /> specifying the tournament whose parent sport to get</param>
        /// <param name="cultures">
        ///     A <see cref="IEnumerable{CultrureInfo}" /> specifying the languages to which the sport must be
        ///     translated
        /// </param>
        /// <param name="fetchTournamentIfMissing">
        ///     Indicates if the tournament should be fetched if not obtained via all
        ///     tournaments request
        /// </param>
        /// <returns>A <see cref="SportData" /> representing the requested sport translated into requested languages</returns>
        private SportData GetSportForTournamentFromCache(URN tournamentId, IEnumerable<CultureInfo> cultures,
            bool fetchTournamentIfMissing)
        {
            var cultureList = cultures as IList<CultureInfo> ?? cultures.ToList();
            ...
            if (!(cachedTournament != null && cachedTournament.HasTranslationsFor(cultureList))) return null;

            CategoryCI cachedCategory;
            if (!(Categories.TryGetValue(cachedTournament.GetCategoryIdAsync().Result, out cachedCategory) &&
                  cachedCategory.HasTranslationsFor(cultureList))) return null;

            SportCI cachedSport;
            if (!(Sports.TryGetValue(cachedCategory.SportId, out cachedSport) &&
                  cachedSport.HasTranslationsFor(cultureList))) return null;

            var category = new CategoryData(
                cachedCategory.Id,
                cachedCategory.Name.Where(k => cultureList.Contains(k.Key))
                    .ToDictionary(kvp => kvp.Key, kvp => kvp.Value),
                cachedCategory.CountryCode,
                new[] {cachedTournament.Id});
            return new SportData(
                cachedSport.Id,
                cachedSport.Name.Where(kvp => cultureList.Contains(kvp.Key))
                    .ToDictionary(kvp => kvp.Key, kvp => kvp.Value),
                new[] {category});
        }

        /// <summary>
        ///     Gets a <see cref="SportData" /> representing the sport specified by <code>id</code> in the languages specified by
        ///     <code>cultures</code>, or a null reference
        ///     if the specified sport does not exist, or it(or one of it's children) is not available in one of the requested
        ///     languages
        /// </summary>
            ...
            lock (_mergeLock)
            {
                ...
                try
                {
                    if (cachedSport.CategoryIds != null)
                    {
                        ...
                        {
                            CategoryCI cachedCategory;
                            if (!(Categories.TryGetValue(categoryId, out cachedCategory) &&
                                  cachedCategory.HasTranslationsFor(cultureList)))
                            {
                                ExecutionLog.Warn(
                                    $"An error occurred while retrieving sport from cache.For sportId = {id} and lang =[{string.Join(",", cultureList)}] we are missing category {categoryId}.");
                                continue;
                            }

                            categories.Add(new CategoryData(
                                cachedCategory.Id,
                                cachedCategory.Name.Where(t => cultureList.Contains(t.Key))
                                    .ToDictionary(kvp => kvp.Key, kvp => kvp.Value),
                                cachedCategory.CountryCode,
                ...
                {
                    ExecutionLog.Warn(
                        $"An error occured while retrieving sport from cache. id={id} and lang=[{string.Join(",", cultureList)}].",
                        e);
                }

                return new SportData(
                    cachedSport.Id,
                    cachedSport.Name.Where(t => cultureList.Contains(t.Key))
                        .ToDictionary(kvp => kvp.Key, kvp => kvp.Value),
                    categories);
            }
        }
        ...
        /// <summary>
        ///     Fetch sport categories for sport specified by <code>id</code> in the languages specified by <code>cultures</code>
        ///     if needed
        /// </summary>
        /// <param name="id">A <see cref="URN" /> specifying the id of the sport</param>
        /// <param name="cultures">
        ///     A <see cref="IEnumerable{CultrureInfo}" /> specifying the languages to which the categories must
        ///     be translated
        /// </param>
        private async Task FetchSportCategoriesIfNeededAsync(URN id, IList<CultureInfo> cultures)
        {
            SportCI cachedSport;
            if (!Sports.TryGetValue(id, out cachedSport)) return;
            ...
            switch (dtoType)
            {
                ...
                case DtoType.Fixture:
                    ...
                    if (fixture?.Tournament != null)
                    {
                        ...
                        AddCategory(fixture.Tournament.Category.Id, fixture.Tournament.Category, fixture.SportId,
                            new List<URN> {fixture.Tournament.Id}, culture);
                    }
                    else
                    {
                ...
                case DtoType.MatchSummary:
                    ...
                    if (match?.Tournament != null)
                    {
                        ...
                        AddCategory(match.Tournament.Category.Id, match.Tournament.Category, match.SportId,
                            new List<URN> {match.Tournament.Id}, culture);
                    }
                    else
                    {
                ...
                case DtoType.RaceSummary:
                    ...
                    if (stageDTO != null)
                    {
                        if (stageDTO.Status != null) AddDataFromSportEventSummary(stageDTO, culture);
                        saved = true;
                    }
                    else
                    {
                ...
                case DtoType.SportList:
                    ...
                    if (sportList != null)
                    {
                        foreach (var s in sportList.Items) AddSport(s.Id, s, culture);
                        saved = true;
                    }
                    else
                    {
                ...
                case DtoType.SportEventSummaryList:
                    ...
                    if (summaryList != null)
                    {
                        foreach (var s in summaryList.Items) AddDataFromSportEventSummary(s, culture);
                        saved = true;
                    }
                    else
                    {
                ...
                case DtoType.TournamentInfo:
                    ...
                    if (tourInfo != null)
                    {
                        ...
                        AddCategory(tourInfo.Category.Id, tourInfo.Category, tourInfo.Sport.Id,
                            new List<URN> {tourInfo.Id}, culture);
                    }
                    else
                    {
                        LogSavingDtoConflict(id, typeof(SportDTO), item.GetType());
                ...
                case DtoType.TournamentSeasons:
                    ...
                    if (tourSeasons?.Tournament != null)
                    {
                        ...
                        AddCategory(tourSeasonsTournament.Category.Id, tourSeasonsTournament.Category,
                            tourSeasonsTournament.Sport.Id, new List<URN> {tourSeasonsTournament.Id}, culture);
                        saved = true;
                    }
                    else
                    {
                ...
                case DtoType.LotteryList:
                    ...
                    if (lotteryList != null)
                    {
                        foreach (var s in lotteryList.Items) AddDataFromSportEventSummary(s, culture);
                        saved = true;
                    }
                    else
                    {
        ...
        private void AddDataFromSportEventSummary(SportEventSummaryDTO dto, CultureInfo culture)
        {
            ...
            if (tour != null)
            {
                ...
                if (tour.TournamentInfo != null) AddDataFromSportEventSummary(tour.TournamentInfo, culture);
                return;
            }
            ...
            if (stage?.Tournament != null)
            {
                ...
                if (stage.Tournament.Category != null)
                    AddCategory(stage.Tournament.Category.Id, stage.Tournament.Category, stage.SportId,
                        new List<URN> {stage.Id}, culture);
                return;
            }
        ...
        private void AddSport(URN id, SportDTO item, CultureInfo culture)
        {
            lock (_mergeLock)
            {
                ...
                {
                    ExecutionLog.Error($"Error saving SportDTO for {id} and lang={culture.TwoLetterISOLanguageName}.",
                        e);
                }
            }
        }
            ...
            lock (_mergeLock)
            {
                try
                {
                    if (Sports.ContainsKey(id))
                    {
                        ...
                        ci?.Merge(
                            new SportCI(
                                new SportDTO(item.Id.ToString(), item.Name, (IEnumerable<tournamentExtended>) null),
                                _dataRouterManager, culture), culture);
                    }
                    else
                    {
                        Sports.Add(id,
                            new SportCI(
                                new SportDTO(item.Id.ToString(), item.Name, (IEnumerable<tournamentExtended>) null),
                                _dataRouterManager, culture));
                    }
                }
                catch (Exception e)
                {
                    ExecutionLog.Error(
                        $"Error saving SportEntityDTO for {id} and lang={culture.TwoLetterISOLanguageName}.", e);
                }
            }
        }
            ...
            lock (_mergeLock)
            {
                try
                {
                    if (Sports.ContainsKey(id))
                    {
                        ...
                        Sports.TryGetValue(id, out ci);
                        ci?.Merge(
                            new SportCI(new SportDTO(item.Sport.Id.ToString(), item.Sport.Name, item.Categories),
                                _dataRouterManager, culture), culture);
                    }
                    else
                    {
            ...
            lock (_mergeLock)
            {
                try
                {
                    if (Categories.ContainsKey(item.Id))
                    {
                        ...
                        ci?.Merge(new CategoryCI(item, culture, item.Tournaments?.FirstOrDefault()?.Sport.Id ?? id),
                            culture);
                    }
                    else
                    {
                        Categories.Add(item.Id,
                            new CategoryCI(item, culture, item.Tournaments?.FirstOrDefault()?.Sport.Id ?? id));
                    }
                }
                catch (Exception e)
                {
                    ExecutionLog.Error(
                        $"Error saving CategoryDTO for {id} and lang={culture.TwoLetterISOLanguageName}.", e);
                }
            }
        ...
        private void AddCategory(URN id, CategorySummaryDTO item, URN sportId, IEnumerable<URN> tournamentIds,
            CultureInfo culture)
        {
            lock (_mergeLock)
            {
                ...
                {
                    ExecutionLog.Error(
                        $"Error saving CategorySummaryDTO for {id} and lang={culture.TwoLetterISOLanguageName}.", e);
                }
            }
        }
        ...
        private void AddCategories(SportCategoriesDTO item, CultureInfo culture)
        {
            lock (_mergeLock)
            {
                foreach (var category in item.Categories)
                    ...
                    {
                        ExecutionLog.Error(
                            $"Error saving CategoryDTO for {category.Id} and lang={culture.TwoLetterISOLanguageName}.",
                            e);
                    }
            }
        }
