...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class LocalizedNamedValueCache : ILocalizedNamedValueCache, IHealthStatusProvider, IDisposable
    {
        /// <summary>
        ...
        ///     A <see cref="ExceptionHandlingStrategy" /> enum member specifying how potential
        ///     exceptions should be handled
        /// </param>
        public LocalizedNamedValueCache(IDataProvider<EntityList<NamedValueDTO>> dataProvider,
            IEnumerable<CultureInfo> cultures, ExceptionHandlingStrategy exceptionStrategy)
        {
        ...
        {
            HealthChecks.RegisterHealthCheck("MatchStatusCache", StartHealthCheck);
        }
        ...
        public HealthCheckResult StartHealthCheck()
        {
            return _namedValues.Any()
                ? HealthCheckResult.Healthy($"MatchStatusCache has {_namedValues.Count} items.")
                : HealthCheckResult.Unhealthy("MatchStatusCache is empty.");
        }
        ...
        {
            _semaphore.Wait(-1);
            if (!_loadedCultures.Any()) GetInternalAsync(new[] {_defaultCultures.First()}).RunSynchronously();
        ...
        public async Task<ILocalizedNamedValue> GetAsync(int id, IEnumerable<CultureInfo> cultures = null)
        {
            ...
            if (cultures == null) cultures = _defaultCultures;

            var cultureList = cultures as IList<CultureInfo> ?? cultures.ToList();
            if (!cultureList.Any()) cultureList = _defaultCultures.ToList();
            ...
            try
            {
                ...
                if (missingCultures.Any()) await GetInternalAsync(missingCultures).ConfigureAwait(false);
        ...
        private async Task FetchAndMerge(CultureInfo culture)
        {
            ...
            CacheLog.Debug(
                $"LocalizedNamedValueCache: {record.Items.Count()} items retrieved for locale '{culture.TwoLetterISOLanguageName}'.");
        }
        ...
        private async Task GetInternalAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                ExecutionLog.Warn(
                    $"An exception occurred while attempting to retrieve match statuses. Exception was: {ex}");
            }
            catch (ObjectDisposedException ex)
            {
                ExecutionLog.Warn(
                    $"GetMarketDescriptionsAsync failed because the instance {ex.ObjectName} is being disposed.");
            }
        }
