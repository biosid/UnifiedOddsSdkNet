...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal abstract class SdkCache : ISdkCache
    {
        ...
        {
            SetDtoTypes();
            if (RegisteredDtoTypes == null) throw new ArgumentNullException(nameof(RegisteredDtoTypes));
        ...
        protected abstract bool CacheAddDtoItem(URN id, object item, CultureInfo culture, DtoType dtoType,
            ISportEventCI requester);
        ...
        {
            var txt =
                $"Invalid data for item id={id}. Expecting: {expectedType?.Name}, received: {receivedType?.Name}.";
            if (logger == null)
