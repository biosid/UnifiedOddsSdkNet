...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching.CI
{
    ...
    public class ReferenceIdCI
    {
        ...
        public void Merge(IDictionary<string, string> referenceIds, bool checkDic)
        {
            if (referenceIds == null) return;
            string val;
            if (referenceIds.TryGetValue("betradar", out val)) int.TryParse(val, out _betradarId);
            if (referenceIds.TryGetValue("BetradarCtrl", out val)) int.TryParse(val, out _betradarId);
            if (referenceIds.TryGetValue("betfair", out val)) int.TryParse(val, out _betfairId);
            if (referenceIds.TryGetValue("rotation_number", out val)) int.TryParse(val, out _rotationNumber);
