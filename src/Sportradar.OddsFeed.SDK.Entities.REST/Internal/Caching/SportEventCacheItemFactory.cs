...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class SportEventCacheItemFactory : ISportEventCacheItemFactory
    {
        ...
        /// <param name="semaphorePool">
        ///     A <see cref="ISemaphorePool" /> instance to be used by instances constructed by this
        ///     factory
        /// </param>
        ...
        public SportEventCacheItemFactory(IDataRouterManager dataRouterManager, ISemaphorePool semaphorePool,
            CultureInfo defaultCulture, MemoryCache fixtureTimestampCache)
        {
        ...
        public SportEventCI Build(SportEventSummaryDTO eventSummary, CultureInfo currentCulture)
        {
                ...
                || eventSummary.Id.TypeGroup == ResourceTypeGroup.TOURNAMENT)
            {
                ...
                if (item != null)
                    return new TournamentInfoCI(item, _dataRouterManager, _semaphorePool, currentCulture,
