...
{
    /// <summary>
    ///     A <see cref="IDeserializer{T}" /> implementation which uses <see cref="XmlElement.LocalName" /> property to
    ///     determine to
    ...
    public class Deserializer<T> : IDeserializer<T> where T : class
    {
        ...
        /// <summary>
        ///     A <see cref="IReadOnlyDictionary{TKey,TValue}" /> containing serializers for all supported types
        /// </summary>
        // ReSharper disable StaticFieldInGenericType
        ...
        static Deserializer()
        {
                ...
                .Where(t => t != baseType && baseType.IsAssignableFrom(t)))
            {
                ...
                var ignoreNamespaceAttribute =
                    feedMessagesType.GetCustomAttribute<OverrideXmlNamespaceAttribute>(false);
                ...
                if (string.IsNullOrWhiteSpace(rootElementName))
                    throw new InvalidOperationException(
                        $"Type {feedMessagesType.FullName} cannot be deserialized with {typeof(Deserializer<>).FullName} because the name of RootXmlElement is not specified");
                if (serializers.ContainsKey(rootElementName))
                    throw new InvalidOperationException(
                        $"Deserializer associated with name {rootElementName} already exists");

                var ignoreNamespace = ignoreNamespaceAttribute?.IgnoreNamespace ?? false;

                serializers.Add(rootElementName,
                    new SerializerWithInfo(new XmlSerializer(feedMessagesType), ignoreNamespace));
            }
        ...
        public T1 Deserialize<T1>(Stream stream) where T1 : T
        {
            using (var reader = new NamespaceIgnorantXmlTextReader(stream))
            {
                ...
                {
                    throw new DeserializationException("The format of the xml is not correct", stream.GetData(), null,
                        ex);
                }

                if (!startElementFound)
                    throw new DeserializationException("Could not retrieve the name of the root element",
                        stream.GetData(), null, null);

                var localName = reader.LocalName;
                SerializerWithInfo serializerWithInfo;
                if (!Serializers.TryGetValue(reader.LocalName, out serializerWithInfo))
                    throw new DeserializationException("Specified root element is not supported", stream.GetData(),
                        localName, null);
                ...
                {
                    throw new DeserializationException("Deserialization failed", stream.GetData(), localName,
        ...
        /// <summary>
        ///     A <see cref="XmlReader" /> derived class, which is capable of deserializing Odds Feed REST messages. Those messages
        ///     have schema issues
