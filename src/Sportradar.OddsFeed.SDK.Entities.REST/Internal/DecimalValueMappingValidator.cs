...
{
    /// <summary>
    ///     A <see cref="IMappingValidator" /> which validates the decimal part of the provided value against a predefined
    ///     value
    /// </summary>
        ...
        public bool Validate(IReadOnlyDictionary<string, string> specifiers)
        {
            ...
            if (!specifiers.TryGetValue(_specifierName, out specifierValue))
                throw new InvalidOperationException(
                    $"Required specifier[{_specifierName}] does not exist in the provided specifiers");

            decimal value;
            if (!decimal.TryParse(specifierValue, NumberStyles.Any, CultureInfo.InvariantCulture, out value))
                throw new InvalidOperationException(
                    $"The value of the specifier {_specifierName}={specifierValue} is not a string representation of a decimal value");
