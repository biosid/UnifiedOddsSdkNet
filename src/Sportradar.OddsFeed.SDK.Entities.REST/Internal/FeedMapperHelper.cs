...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal
{
    ...
    {
        /// <summary>
        ///     Creates and returns a <see cref="IReadOnlyDictionary{TKey, TValue}" /> by splitting values provided by the
        ///     <code>valueParts</code>
        /// </summary>
            ...
            foreach (var specifier in values)
            {
                ...
                if (specifierArray.Length != 2)
                    throw new FormatException(
                        $"Format of the specifier {specifier} is not correct. It must have a key and value separated by = sign");
                var key = specifierArray[0].Trim();
                var value = specifierArray[1].Trim();
                if (string.IsNullOrEmpty(key))
                    throw new FormatException(
                        $"Format of the specifier {specifier} is not correct. The key must not be empty");
                if (string.IsNullOrEmpty(value))
                    throw new FormatException(
                        $"Format of the specifier {specifier} is not correct. The value must not be empty");
                if (tuples.Any(t => t.Item1 == key))
                    throw new ArgumentException($"Duplicated specifiers are not allowed. specifier={key}",
                        nameof(values));
        ...
        /// <summary>
        ///     Creates a <see cref="IReadOnlyDictionary{TKey,TValue}" /> from a <see cref="string" /> containing string
        ///     representation of validity attributes
        /// </summary>
