...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    ...
    public class MarketMappingProvider : IMarketMappingProvider
    {
        ...
        {
            if (_producer.Equals(_producerManager.Get(5))) return null;
            ...
            if (marketMappings == null || !marketMappings.Any()) return null;
        ...
        private async Task<IMarketDescription> GetMarketDescriptorAsync(IEnumerable<CultureInfo> cultures)
        {
            try
            {
                ...
                var marketDescriptor = await _marketCacheProvider
                    .GetMarketDescriptionAsync(_marketId, _specifiers, cultures, true).ConfigureAwait(false);
            ...
            {
                HandleMappingErrorCondition("Failed to retrieve market name descriptor", "MarketId",
                    _marketId.ToString(), string.Empty, ex);
                return null;
            }
        }
        ...
        {
            if (marketDescription == null) return null;
        ...
        {
            if (string.IsNullOrEmpty(sovTemplate) || !sovTemplate.Contains("{")) return sovTemplate;
        ...
        private string SwitchSpecifierPlaceHolder(string currentTemplate)
        {
            ...
            var sovValue = sovKey.Contains("$")
                ? HandleSpecialSovKeys(sovKey)
                : _specifiers.FirstOrDefault(s => s.Key == sovKey).Value;

            if (string.IsNullOrEmpty(sovValue))
                HandleMappingErrorCondition(
                    $"Market with id:{_marketId}, producer:{_producer}, sportId:{_sportId} and SOV:{currentTemplate} has no appropriate specifier.",
                    "SovTemplate", currentTemplate, "MarketMapping", null);
            else
        ...
        private string HandleSpecialSovKeys(string sovKey)
        {
            ...
            if (sovKey == "$score")
            {
                ...
                if (eventStatus != null) result = $"{eventStatus.HomeScore ?? 0}:{eventStatus.AwayScore ?? 0}";
            }

            return result;
        }

        private void HandleMappingErrorCondition(string message, string propertyName, string propertyValue,
            string targetTypeName, Exception innerException)
        {
            ...
            if (!string.IsNullOrEmpty(targetTypeName)) sb.Append(", TargetType=[").Append(targetTypeName);
