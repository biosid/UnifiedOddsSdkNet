...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    internal class MarketMappingCacheItem
    {
        ...
        public static MarketMappingCacheItem Build(MarketMappingDTO dto, IMappingValidatorFactory factory,
            CultureInfo culture)
        {
            Contract.Requires(dto != null);
        ...
        {
            if (dto.OutcomeMappings == null) return;

            var shouldHave = OutcomeMappings.Count > 0;

            if (string.IsNullOrEmpty(dto.OrgMarketId)) OrgMarketId = dto.OrgMarketId;

            foreach (var outcomeMappingDTO in dto.OutcomeMappings)
            {
                var mapping = OutcomeMappings.FirstOrDefault(f =>
                    f.OutcomeId.Equals(outcomeMappingDTO.OutcomeId, StringComparison.InvariantCultureIgnoreCase));
