...
{
    /// <summary>
    ///     A <see cref="IMarketDescriptionCache" /> implementation which provides access to correct cache by examining the
    ///     market specifiers
    /// </summary>
    ...
    public class MarketCacheProvider : IMarketCacheProvider
    {
        ...
        /// <summary>
        ///     Gets a <see cref="IMarketDescription" /> instance for the market specified by <code>id</code> and
        ...
        public async Task<IMarketDescription> GetMarketDescriptionAsync(int marketId,
            IReadOnlyDictionary<string, string> specifiers, IEnumerable<CultureInfo> cultures,
            bool fetchVariantDescriptions)
        {
            ...
            try
            {
                ...
                marketDescriptor = await _invariantMarketsCache.GetMarketDescriptionAsync(marketId, null, cultureInfos)
            ...
            {
                throw new CacheItemNotFoundException($"Market description with market id {marketId} could not be found",
                    marketId.ToString(), e);
            }
            ...
            if (string.IsNullOrEmpty(variantValue)) return marketDescriptor;
            ...
            if (fetchVariantDescriptions)
            {
                ...
                if (IsMarketOutcomeText(marketDescriptor) || IsMarketPlayerProps(marketDescriptor))
                    return await ProvideDynamicVariantEndpointMarketAsync(marketId, cultureInfos, marketDescriptor,
                        variantValue).ConfigureAwait(false);

                // case 3: "normal" variant market available on the full variant market list (static)
                var marketDesc =
                    await ProvideFullVariantListEndpointMarketAsync(marketId, cultureInfos, marketDescriptor,
                        variantValue).ConfigureAwait(false);
                if (marketDesc == null)
                    // case 4: dynamic market which is not defined
                    marketDesc =
                        await ProvideDynamicVariantEndpointMarketAsync(marketId, cultureInfos, marketDescriptor,
                            variantValue).ConfigureAwait(false);

                if (marketDesc?.Mappings != null && marketDesc.Mappings.Any())
                {
                    var marketDescImpl = (MarketDescription) marketDesc;
                    marketDescImpl.Mappings = marketDescImpl.Mappings.Where(s =>
                        s.MarketId.Equals(marketId.ToString()) || s.MarketId.StartsWith($"{marketId}:")).ToList();
        ...
        public async Task<bool> ReloadMarketDescriptionAsync(int marketId,
            IReadOnlyDictionary<string, string> specifiers, string sourceCache)
        {
            ...
            try
            {
                ...
                if (sourceCache.Equals("VariantMarketDescriptionCache", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    specifiers?.TryGetValue(SdkInfo.VariantDescriptionName, out variantValue);
                    _executionLog.Debug(
                        $"Reloading variant market description for market={marketId} and variant={variantValue}");
                    var variantMarketDescriptionCache = (VariantMarketDescriptionCache) _variantMarketsCache;
                    variantMarketDescriptionCache.CacheDeleteItem(
                        VariantMarketDescriptionCache.GetCacheKey(marketId, variantValue),
            ...
            _executionLog.Warn(
                $"Calling ReloadMarketDescriptionAsync with unknown sourceCache={sourceCache}. (marketId={marketId})");
            return false;
        }
            ...
            IList<CultureInfo> locales, IMarketDescription marketDescriptor, string variantValue)
        {
            try
            {
                ...
                if (variantDescription == null) return null;
            ...
            IList<CultureInfo> locales, IMarketDescription marketDescriptor, string variantValue)
        {
            ...
            try
            {
                variantDescriptor = await _variantMarketsCache
                    .GetMarketDescriptionAsync(marketId, variantValue, locales).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                var langs = string.Join(",", locales.Select(s => s.TwoLetterISOLanguageName));
                _executionLog.Warn(
                    $"There was an error providing the explicit variant market description -> marketId:{marketId}, variantValue: {variantValue}, locales: [{langs}]",
                    e);
            }
