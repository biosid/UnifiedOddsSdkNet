...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    ...
    public class NameProvider : INameProvider
    {
        ...
        ///     A <see cref="INameExpressionFactory" /> instance used to built
        ///     <see cref="INameExpression" /> instances
        /// </param>
        ...
        public async Task<string> GetMarketNameAsync(CultureInfo culture)
        {
            ...
            if (nameDescription == null)
            {
                HandleErrorCondition(
                    "Retrieved market descriptor does not contain name descriptor in the specified language", null,
                    null, culture, null);
                return null;
            }
            ...
            if (expressions == null) return nameDescription;
            ...
            {
                HandleErrorCondition("Error occurred while evaluating the name expression", null, nameDescription,
                    culture, ex);
                return null;
            }
        ...
        {
            if (outcomeId.StartsWith(SdkInfo.PlayerProfileMarketPrefix) ||
                outcomeId.StartsWith(SdkInfo.CompetitorProfileMarketPrefix))
                try
                {
            ...
            var marketDescriptor =
                await GetMarketDescriptionForOutcomeAsync(outcomeId, culture, true).ConfigureAwait(false);
            ...
            if (string.IsNullOrEmpty(nameDescription))
            {
                HandleErrorCondition(
                    ...
                    outcomeId, null, culture, null);
                return null;
            }

            if (marketDescriptor.Attributes?.FirstOrDefault(a => a.Name == SdkInfo.FlexScoreMarketAttributeName) !=
                null)
                try
                {
                ...
                {
                    HandleErrorCondition("The generation of name for flex score market outcome failed", outcomeId,
                        nameDescription, culture, ex);
                }
            ...
            catch (Exception ex)
            {
                if (ex is ArgumentException || ex is FormatException)
                {
                    HandleErrorCondition("The name description parsing failed", outcomeId, nameDescription, culture,
                        ex);
                    return null;
                }
            ...
            if (expressions == null || !expressions.Any()) return nameDescription;
            ...
            {
                HandleErrorCondition("Error occurred while evaluating the name expression", outcomeId, nameDescription,
                    culture, ex);
                return null;
            }
        ...
        {
            var idParts = outcomeId.Split(new[] {SdkInfo.NameProviderCompositeIdSeparator},
            ...
            foreach (var idPart in idParts)
            {
                ...
                try
                {
                    ...
                    if (idPart.StartsWith(SdkInfo.PlayerProfileMarketPrefix))
                    {
                        ...
                        if (!_competitorsAlreadyFetched)
                        {
                            ...
                            if (competitionEvent != null)
                                ...
                                {
                                    var competitors =
                                        await competitionEvent.GetCompetitorsAsync().ConfigureAwait(false);
                                    var competitorsList = competitors.ToList();
                                    if (!competitorsList.Any()) continue;
                                    var tasks = competitorsList.Select(s =>
                                        _profileCache.GetCompetitorProfileAsync(s.Id, fetchCultures));
                                    await Task.WhenAll(tasks).ConfigureAwait(false);
                                }
                                catch (Exception ex)
                                {
                        ...
                        var profile = await _profileCache.GetPlayerProfileAsync(profileId, fetchCultures)
                    ...
                    {
                        var profile = await _profileCache.GetCompetitorProfileAsync(profileId, fetchCultures)
                            .ConfigureAwait(false);
                        names.Add(profile.GetName(culture));
                        continue;
                    }
        ...
        /// <summary>
        ///     Asynchronously gets a <see cref="IMarketDescription" /> instance describing the specified market in the specified
        ///     language
        /// </summary>
        ...
        private async Task<IMarketDescription> GetMarketDescriptorAsync(CultureInfo culture)
        {
            ...
            return await _marketCacheProvider.GetMarketDescriptionAsync(_marketId, _specifiers, new[] {culture}, true)
                .ConfigureAwait(false);
        }

        /// <summary>
        ///     Handles the error condition occurred during name generation either by logging or throwing an exception - depending
        ///     on the <see cref="_exceptionStrategy" />
        /// </summary>
        ...
        private void HandleErrorCondition(string message, string outcomeId, string nameDescriptor, CultureInfo culture,
            Exception innerException)
        {
            ...
            var specifiersString = _specifiers == null
                ? "null"
                : string.Join(SdkInfo.SpecifiersDelimiter, _specifiers.Select(k => $"{k.Key}={k.Value}"));
            sb.Append(" MarketId=").Append(_marketId).Append(" Specifiers=[").Append(specifiersString).Append("]");

            if (outcomeId != null) sb.Append(" OutcomeId=").Append(outcomeId);
            sb.Append("]").Append(" Culture=").Append(culture.TwoLetterISOLanguageName);

            if (nameDescriptor != null) sb.Append(" Retrieved nameDescriptor=[").Append(nameDescriptor);
            ...
            if (_exceptionStrategy == ExceptionHandlingStrategy.THROW)
                throw new NameGenerationException(message, _marketId, _specifiers, outcomeId, nameDescriptor, culture,
                    innerException);
        }
        ...
        /// <param name="nameDescriptorFormat">
        ///     When the call completes, the <code>nameDescriptor</code> replaced with string format
        ///     placeholders
        /// </param>
        ...
        protected IList<INameExpression> GetNameExpressions(string nameDescriptor, out string nameDescriptorFormat)
        {
            ...
            if (expressionStrings == null) return null;
        ...
        private async Task<IMarketDescription> GetMarketDescriptionForOutcomeAsync(string outcomeId,
            CultureInfo culture, bool firstTime)
        {
            IMarketDescription marketDescriptor;
            try
            {
            ...
            if (marketDescriptor.Outcomes == null)
            {
                if (firstTime)
                {
                    HandleErrorCondition("Retrieved market descriptor is lacking outcomes", outcomeId, null, culture,
                        null);
                    if (((MarketDescription) marketDescriptor).MarketDescriptionCI.CanBeFetched())
                    {
                        ...
                        return await GetMarketDescriptionForOutcomeAsync(outcomeId, culture, false)
                            .ConfigureAwait(false);
                    }
                }

                HandleErrorCondition("Retrieved market descriptor does not contain outcomes", outcomeId, null, culture,
                    null);
                return null;
            }
            ...
            if (outcome == null)
            {
                if (firstTime)
                {
                    HandleErrorCondition("Retrieved market descriptor is missing outcome", outcomeId, null, culture,
                        null);
                    if (((MarketDescription) marketDescriptor).MarketDescriptionCI.CanBeFetched())
                    {
                        HandleErrorCondition("Reloading market description", outcomeId, null, culture, null);
                ...
                HandleErrorCondition("Retrieved market descriptor does not contain outcome", outcomeId, null, culture,
                    null);
                return null;
            }
