...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    internal class MarketDescriptionCacheItem
    {
        ...
        {
            if (!string.IsNullOrEmpty(source)) SourceCache = source;
        ...
        private bool MarketMappingsMatch(MarketMappingCacheItem ci, MarketMappingDTO dto)
        {
            ...
            if (isMatch && ci.SportId != null) isMatch = ci.SportId.Equals(dto.SportId);

            if (isMatch && ci.ProducerIds != null) isMatch = ci.ProducerIds.All(a => dto.ProducerIds.Contains(a));
        ...
        {
            var names = _names.Aggregate(string.Empty,
                (current, name) => current + $", {name.Key.TwoLetterISOLanguageName}-{name.Value}").Substring(2);
            var desc = _descriptions.Aggregate(string.Empty,
                (current, d) => current + $", {d.Key.TwoLetterISOLanguageName}-{d.Value}");
            ...
            ExecutionLog.Debug(
                $"Original Id={Id}, Names=[{names}], Descriptions=[{desc}], Variant=[{Variant}], Specifiers=[{specs}], Outocomes=[{outcomes}], Mappings=[{maps}].");
            ...
            ExecutionLog.Debug(
                $"New Id={dto.Id}, Name=[{culture.TwoLetterISOLanguageName}-{dto.Name}], Descriptions=[{dto.Description}], Variant=[{dto.Variant}], Specifiers=[{specsNew}], Outocomes=[{outcomesNew}], Mappings=[{mapsNew}].");
        }
    }
}