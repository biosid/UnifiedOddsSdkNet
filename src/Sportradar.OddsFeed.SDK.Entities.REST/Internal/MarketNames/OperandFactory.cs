...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    ...
    public class OperandFactory : IOperandFactory
    {
        ...
        ///     A <see cref="IReadOnlyDictionary{String,String}" /> representing specifiers for the market
        ///     associated with the constructed operand.
        /// </param>
        ...
        public IOperand BuildOperand(IReadOnlyDictionary<string, string> specifiers, string operandExpression)
        {
            if (operandExpression.StartsWith("(") && !operandExpression.EndsWith(")") ||
                ...
                throw new FormatException(
                    $"Format of the operand {operandExpression} is not correct. It contains un-closed parenthesis");
            ...
            if (operandExpression.StartsWith("(") && operandExpression.EndsWith(")"))
            {
                ...
                {
                    throw new FormatException(
                        $"Format of operand {operandExpression} is not correct. It does not contain an operation identifier");
                }

                if (parts.Length != 2)
                    throw new FormatException(
                        $"Format of operand {operandExpression} is not correct. It contains more than one operation identifier");
