...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    ...
    internal static class NameExpressionHelper
    {
        ...
        /// <param name="descriptorFormat">
        ///     When the call is completed it contains a provided descriptor where names are replaced
        ///     with format placeholders e.g. {0}
        /// </param>
        ...
        ///     The specified specifier does not exist or it's value is not string
        ///     representation of int
        /// </exception>
        internal static void ParseSpecifier(string specifierName, IReadOnlyDictionary<string, string> specifiers,
            out int specifierValue)
        {
            ...
            if (!int.TryParse(specifierValueString, out specifierValue))
                throw new InvalidOperationException(
                    $"Specifier[key={specifierName}, value={specifierValueString}] must be a string representation of a {typeof(int).FullName} type");
        }
            ...
            out decimal specifierValue)
        {
            ...
            if (!decimal.TryParse(specifierValueString, NumberStyles.Number, new CultureInfo("en").NumberFormat,
                out specifierValue))
                throw new InvalidOperationException(
                    $"Specifier[key={specifierName}, value={specifierValueString}] must be a string representation of a {typeof(decimal).FullName} type");
        }
    }
}