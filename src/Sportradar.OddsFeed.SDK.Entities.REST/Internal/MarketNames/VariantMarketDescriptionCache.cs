...
{
    /// <summary>
    ///     A <see cref="IMarketDescriptionCache" /> implementation used to store market descriptors for variant markets
    ///     (single - dynamic)
    /// </summary>
    ...
    internal class VariantMarketDescriptionCache : SdkCache, IMarketDescriptionCache, IDisposable, IHealthStatusProvider
    {
        ...
        private readonly ConcurrentDictionary<string, DateTime> _fetchedVariants =
        ...
        {
            HealthChecks.RegisterHealthCheck("VariantMarketDescriptorCache", StartHealthCheck);
        }
        ...
        public async Task<IMarketDescription> GetMarketDescriptionAsync(int marketId, string variant,
            IEnumerable<CultureInfo> cultures)
        {
            ...
            catch (Exception ex)
            {
                if (ex is CommunicationException || ex is DeserializationException || ex is MappingException)
                    throw new CacheItemNotFoundException("The requested key was not found in the cache",
                        GetCacheKey(marketId, variant), ex);
                throw;
            }
        ...
        /// <returns>
        ///     The <see cref="MarketDescriptionCacheItem" /> retrieved from the cache or a null reference if item is not
        ///     found
        /// </returns>
        ...
        /// <summary>
        ///     Gets a <see cref="IEnumerable{CultureInfo}" /> containing <see cref="CultureInfo" /> instances from provided
        ///     <code>requiredTranslations</code>
        ...
        /// <returns>
        ///     A <see cref="IEnumerable{CultureInfo}" /> containing missing translations or a null reference if none of the
        ///     translations are missing
        /// </returns>
        private static IEnumerable<CultureInfo> GetMissingTranslations(MarketDescriptionCacheItem item,
            IEnumerable<CultureInfo> requiredTranslations)
        {
            Contract.Requires(requiredTranslations != null && requiredTranslations.Any());

            if (item == null) return requiredTranslations;
        ...
        /// <summary>
        ///     Asynchronously gets the specified <see cref="MarketDescriptionCacheItem" />. If the item is not found in local
        ///     cache, it is fetched and stored / merged into cache
        /// </summary>
        ...
        private async Task<MarketDescriptionCacheItem> GetMarketInternal(int id, string variant,
            IEnumerable<CultureInfo> cultures)
        {
            Contract.Requires(!string.IsNullOrEmpty(variant));
            ...
            if (GetMissingTranslations(description, cultureList) == null) return description;
            ...
            try
            {
                ...
                if (!IsFetchingAllowed(GetCacheKey(id, variant))) return description;

                var cultureTaskDictionary = missingLanguages.ToDictionary(c => c,
                    c => _dataRouterManager.GetVariantMarketDescriptionAsync(id, variant, c));
        ...
        {
            if (!_fetchedVariants.ContainsKey(cacheId)) return true;
