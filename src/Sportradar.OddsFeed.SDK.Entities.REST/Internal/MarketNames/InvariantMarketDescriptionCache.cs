...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.MarketNames
{
    ...
    internal class InvariantMarketDescriptionCache : SdkCache, IMarketDescriptionCache, IDisposable,
        IHealthStatusProvider
    {
        ...
        public async Task<bool> LoadMarketDescriptionsAsync()
        {
            ...
            {
                ExecutionLog.Debug(
                    $"Loading invariant market descriptions for [{string.Join(",", _prefetchLanguages.Select(l => l.TwoLetterISOLanguageName))}] (user request).");
        ...
        {
            if (!_isDisposed) _semaphoreCacheMerge.Wait();
        ...
        public async Task<IEnumerable<IMarketDescription>> GetAllInvariantMarketDescriptionsAsync(
            IEnumerable<CultureInfo> cultures)
        {
            var cultureList = cultures as List<CultureInfo> ?? cultures.ToList();
