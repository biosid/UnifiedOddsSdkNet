...
{
    /// <summary>
    ///     A <see cref="IVariantDescriptionCache" /> implementation used to store <see cref="IVariantDescription" /> for
    ///     variant markets (static)
    /// </summary>
    ...
    internal class VariantDescriptionListCache : SdkCache, IVariantDescriptionCache, IDisposable, IHealthStatusProvider
    {
        ...
        /// <summary>
        ///     A <see cref="IList{T}" /> used to store languages for which the data was already fetched (al least once)
        /// </summary>
        private readonly IList<CultureInfo> _fetchedLanguages = new List<CultureInfo>();

        /// <summary>
        ///     The <see cref="IMappingValidatorFactory" /> used to construct <see cref="IMappingValidator" /> instances for market
        ///     mappings
        /// </summary>
        ...
        {
            HealthChecks.RegisterHealthCheck("VariantDescriptionListCache", StartHealthCheck);
        }
        ...
        public async Task<IVariantDescription> GetVariantDescriptorAsync(string variantId,
        ...
        public async Task<bool> LoadMarketDescriptionsAsync()
        {
            ...
            {
                ExecutionLog.Debug(
                    $"Loading variant market descriptions for [{string.Join(", ", _prefetchLanguages.Select(l => l.TwoLetterISOLanguageName))}] (user request).");
            ...
            {
                ExecutionLog.Warn(
                    $"An error occurred while fetching market descriptions variant list. The exception:{ex.Message}");
                return false;
            }
        ...
        private async void OnTimerElapsed(object sender, EventArgs e)
        {
            ...
            try
            {
                _fetchedLanguages.Clear();
                ExecutionLog.Debug(
                    $"Loading variant market descriptions for [{string.Join(",", languagesToFetch.Select(l => l.TwoLetterISOLanguageName))}] (timer).");
            ...
            catch (Exception ex)
            {
                if (ex is CommunicationException || ex is DeserializationException || ex is FormatException)
                {
                    ...
                    ExecutionLog.Warn(
                        $"An error occurred while periodically fetching variant market description of languages {languagesString}",
                        ex);
                    return;
                }
                ...
                {
                    ExecutionLog.Warn(
                        $"An error occurred while periodically fetching variant market descriptions because the object graph is being disposed. Object causing the exception:{disposedException.ObjectName}");
                    return;
                }
                ...
                {
                    ExecutionLog.Warn(
                        "An error occurred while periodically fetching variant market descriptions because the object graph is being disposed.");
                    return;
                }
        ...
        /// <returns>
        ///     The <see cref="VariantDescriptionCacheItem" /> retrieved from the cache or a null reference if item is not
        ///     found
        /// </returns>
        private VariantDescriptionCacheItem GetItemFromCache(string id)
        {
            ...
            if (!_isDisposed) _semaphoreCacheMerge.Release();
            return (VariantDescriptionCacheItem) cacheItem?.Value;
        }

        /// <summary>
        ///     Asynchronously gets the <see cref="VariantDescriptionCacheItem" /> specified by it's id. If the item is not found
        ///     in local cache, all items for specified
        ...
        private async Task<VariantDescriptionCacheItem> GetVariantDescriptionInternalAsync(string id,
            IEnumerable<CultureInfo> cultures)
        {
            ...
            if ((description = GetItemFromCache(id)) != null &&
                !LanguageHelper.GetMissingCultures(cultureList, description.FetchedLanguages).Any()) return description;
            try
            {
                ...
                if (!missingLanguages.Any()) return description;

                var cultureTaskDictionary =
                    missingLanguages.ToDictionary(l => l, l => _dataRouterManager.GetVariantDescriptionsAsync(l));
            ...
            return (description = GetItemFromCache(id)) != null &&
