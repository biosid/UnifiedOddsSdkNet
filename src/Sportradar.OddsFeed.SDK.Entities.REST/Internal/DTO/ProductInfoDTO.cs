    ...
    internal class ProductInfoDTO
    {
        internal ProductInfoDTO(productInfo productInfo)
        {
            ...
            ProductInfoLinks = productInfo.links != null && productInfo.links.Any()
                ? new ReadOnlyCollection<ProductInfoLinkDTO>(productInfo.links.Select(p => new ProductInfoLinkDTO(p))
                    .ToList())
                : null;

            StreamingChannels = productInfo.streaming != null && productInfo.streaming.Any()
                ? new ReadOnlyCollection<StreamingChannelDTO>(productInfo.streaming
                    .Select(s => new StreamingChannelDTO(s)).ToList())
                : null;
        }
