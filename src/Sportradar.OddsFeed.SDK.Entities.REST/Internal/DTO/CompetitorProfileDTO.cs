...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.DTO
{
    ...
    public class CompetitorProfileDTO
    {
        ...
        /// <summary>
        ///     Gets a <see cref="IEnumerable{T}" /> representing players which are part of the represented competitor
        /// </summary>
        ...
        public CompetitorProfileDTO(competitorProfileEndpoint record)
        {
            ...
            if (record.players != null && record.players.Any())
                Players = new ReadOnlyCollection<PlayerProfileDTO>(record.players.Select(p => new PlayerProfileDTO(p))
                    .ToList());
            if (record.jerseys != null)
                Jerseys = new ReadOnlyCollection<JerseyDTO>(record.jerseys.Select(p => new JerseyDTO(p)).ToList());
            if (record.manager != null) Manager = new ManagerDTO(record.manager);
            if (record.venue != null) Venue = new VenueDTO(record.venue);
        }
