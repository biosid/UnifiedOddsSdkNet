...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.DTO
{
    ...
    public static class RestMapperHelper
    {
        ...
        /// <summary>
        ///     It checks if there are exactly 2 competitors and one is home and seconds away. If so, it returns dictionary
        ///     indicating which of team is home and which away; else null
        /// </summary>
        /// <param name="competitors">The competitors to be checked</param>
        /// <returns>IDictionary&lt;HomeAway, URN&gt;</returns>
        public static IDictionary<HomeAway, URN> FillHomeAwayCompetitors(
            IReadOnlyCollection<teamCompetitor> competitors)
        {
            if (competitors == null || competitors.Count != 2) return null;
        ...
        {
            if (!restTypeSpecified) return null;
            if (bonusDrumType == bonusDrumType.same) return BonusDrumType.Same;
        ...
        {
            if (!restTypeSpecified) return DrawType.Unknown;
            if (drawType == drawType.drum) return DrawType.Drum;
            return DrawType.Rng;
        }
        ...
        {
            if (!restTypeSpecified) return DrawStatus.Unknown;
            if (drawStatus == drawStatus.open) return DrawStatus.Open;
            if (drawStatus == drawStatus.canceled) return DrawStatus.Cancelled;
            if (drawStatus == drawStatus.closed) return DrawStatus.Closed;
            if (drawStatus == drawStatus.finished) return DrawStatus.Finished;
        ...
        {
            if (!restTypeSpecified) return TimeType.Unknown;
            if (timeType == timeType.interval) return TimeType.Interval;
        ...
        /// </summary>
        /// <param name="value">
        ///     A <see cref="string" /> representation of the <see cref="BookingStatus" />, or a null reference. Note that mapping
        ///     of a null
        ///     reference is allowed and result of mapping is a null reference
        /// </param>
        /// <param name="result">
        ///     When the call is completed it contains <see cref="BookingStatus" /> which was obtained by mapping or a null
        ///     reference if a null reference
        ///     was provided or mapping failed
        /// </param>
        /// <returns>
        ///     True if the provided <see cref="string" /> was successfully mapped to <see cref="BookingStatus" />; False
        ///     otherwise
        /// </returns>
        ...
        /// </summary>
        /// <param name="value">
        ///     A <see cref="string" /> representation of the <see cref="CoveredFrom" />, or a null reference. Note that mapping of
        ///     a null
        ///     reference is allowed and result of mapping is a null reference
        /// </param>
        /// <param name="result">
        ///     When the call is completed it contains <see cref="CoveredFrom" /> which was obtained by mapping or a null reference
        ///     if a null reference
        ///     was provided or mapping failed
        /// </param>
        /// <returns>
        ///     True if the provided <see cref="string" /> was successfully mapped to <see cref="CoveredFrom" />; False
        ///     otherwise
        /// </returns>
