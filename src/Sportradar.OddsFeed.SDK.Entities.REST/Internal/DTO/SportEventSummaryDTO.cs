...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.DTO
{
    ...
    public class SportEventSummaryDTO
    {
        ...
        internal SportEventSummaryDTO(sportEvent sportEvent)
        {
            ...
            {
                URN sportId;
                if (URN.TryParse(sportEvent.tournament.sport.id, out sportId)) SportId = sportId;
            }

            Name = sportEvent.name;
            SportEventType? type;
            if (RestMapperHelper.TryGetSportEventType(sportEvent.type, out type)) Type = type;
            if (!string.IsNullOrEmpty(sportEvent.replaced_by))
            {
                URN replacedBy;
                if (URN.TryParse(sportEvent.replaced_by, out replacedBy)) ReplacedBy = replacedBy;
            }
        ...
        protected SportEventSummaryDTO(parentStage parentStage)
        {
            ...
            SportEventType? type;
            if (RestMapperHelper.TryGetSportEventType(parentStage.type, out type)) Type = type;
            if (!string.IsNullOrEmpty(parentStage.replaced_by))
            {
                URN replacedBy;
                if (URN.TryParse(parentStage.replaced_by, out replacedBy)) ReplacedBy = replacedBy;
            }
        ...
        protected SportEventSummaryDTO(sportEventChildrenSport_event childStage)
        {
            ...
            SportEventType? type;
            if (RestMapperHelper.TryGetSportEventType(childStage.type, out type)) Type = type;
            if (!string.IsNullOrEmpty(childStage.replaced_by))
            {
                URN replacedBy;
                if (URN.TryParse(childStage.replaced_by, out replacedBy)) ReplacedBy = replacedBy;
            }
        ...
        /// <summary>
        ///     Gets a <see cref="DateTime" /> specifying the scheduled start date for the sport event associated with the current
        ///     instance
        /// </summary>
        public DateTime? Scheduled { get; }

        /// <summary>
        ///     Gets a <see cref="DateTime" /> specifying the scheduled end time of the sport event associated with the current
        ///     instance
        /// </summary>
        ...
        /// <summary>
        ///     Gets a <see cref="SportEventType" /> specifying the type of the associated sport event or a null reference if
