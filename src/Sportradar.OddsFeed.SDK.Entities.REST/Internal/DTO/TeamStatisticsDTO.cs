    ...
    public class TeamStatisticsDTO
    {
        internal TeamStatisticsDTO(teamStatistics statistics, IDictionary<HomeAway, URN> homeAwayCompetitors)
        {
            ...
            if (TeamId != null && homeAwayCompetitors != null)
            {
                ...
                if (x.Any()) HomeOrAway = x.First().Key == HomeAway.Home ? HomeAway.Home : HomeAway.Away;
            }

            if (statistics.statistics != null)
            {
                int tmp;
                if (int.TryParse(statistics.statistics.yellow_cards, out tmp)) YellowCards = tmp;
                if (int.TryParse(statistics.statistics.red_cards, out tmp)) RedCards = tmp;
                if (int.TryParse(statistics.statistics.yellow_red_cards, out tmp)) YellowRedCards = tmp;
                if (int.TryParse(statistics.statistics.cards, out tmp)) Cards = tmp;
                if (int.TryParse(statistics.statistics.corner_kicks, out tmp)) CornerKicks = tmp;
            }
        }

        internal TeamStatisticsDTO(HomeAway? homeAway, int yellowCards, int redCards, int yellowRedCards,
            int cornerKicks)
        {
