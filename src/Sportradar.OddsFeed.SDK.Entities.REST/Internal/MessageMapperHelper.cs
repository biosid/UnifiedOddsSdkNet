...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal
{
    ...
    public static class MessageMapperHelper
    {
        ...
        {
            if (!specified) return VoidFactor.Zero;

            if (value < 0.1) return VoidFactor.Zero;
            if (value > 0.4 && value < 0.6) return VoidFactor.Half;
            if (value > 0.9) return VoidFactor.One;
        ...
        public static bool IsEnumMember<TEnum>(object value) where TEnum : struct
        {
            ...
            if (!enumType.IsEnum)
                throw new InvalidOperationException(
                    $"Type specified by generic parameter T must be an enum. Provided type was:{typeof(TEnum).FullName}");
        ...
        /// <summary>
        ///     Converts the provided int <code>value</code> to the member of the specified enum, or returns
        ...
        public static T GetEnumValue<T>(string value) where T : struct, IConvertible
        {
            ...
            foreach (int v in enumValues)
            {
                ...
                if (string.Equals(enumChoice.ToString(CultureInfo.InvariantCulture), value,
                        StringComparison.InvariantCultureIgnoreCase)
                    || string.Equals(enumChoice.ToString(CultureInfo.InvariantCulture), trimmedValue,
                        StringComparison.InvariantCultureIgnoreCase))
                    return enumChoice;
            }
        ...
        /// <returns>The member of the specified enum</returns>
        public static T GetEnumValue<T>(string value, T defaultValue, IDictionary<string, T> predefinedPairs)
