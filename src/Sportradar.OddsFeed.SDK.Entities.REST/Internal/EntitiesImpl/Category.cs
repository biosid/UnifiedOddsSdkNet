...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.EntitiesImpl
{
    ...
    internal class Category : CategorySummary, ICategory
    {
        ...
        /// <param name="names">a <see cref="IReadOnlyDictionary{TKey,TValue}" /> containing translated category name</param>
        /// <param name="countryCode">a country code</param>
        /// <param name="tournaments">
        ///     a <see cref="IEnumerable{ISportEvent}" /> representing the tournaments which belong to the
        ///     category represented by the current instance
        /// </param>
        public Category(URN id, IReadOnlyDictionary<CultureInfo, string> names, string countryCode,
            IEnumerable<ISportEvent> tournaments)
            : base(id, names, countryCode)
        {
            Contract.Requires(tournaments != null);

            Tournaments = tournaments as IReadOnlyCollection<ISportEvent> ??
                          new ReadOnlyCollection<ISportEvent>(tournaments.ToList());
        }
        ...
        protected override string PrintF()
        {
            var names = Names == null ? string.Empty : string.Join(", ", Names.Keys.Select(k => $"{k}={Names[k]}"));
            //var tournaments = string.Join(" ,", Tournaments.Select(k => k.Id.ToString()));
            var tournaments = Tournaments.Aggregate(string.Empty,
                (current, t) => current + $"\t {((Tournament) t).ToString("f")}");
            return $"Id={Id}, Names=[{names}], Tournaments=[{tournaments}]";
        }
