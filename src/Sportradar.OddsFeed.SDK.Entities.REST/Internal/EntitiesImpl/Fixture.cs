    ...
    internal class Fixture : EntityPrinter, IFixture
    {
        internal Fixture(FixtureDTO fixtureDto)
        {
            ...
            if (fixtureDto.TvChannels != null)
            {
                if (TvChannels == null)
                {
                    TvChannels = fixtureDto.TvChannels.Select(tvChannelDTO =>
                        new TvChannel(tvChannelDTO.Name, tvChannelDTO.StartTime, tvChannelDTO.StreamUrl)).ToList();
                }
                else
                {
                    ...
                    {
                        var tvChannel = tvChannels.Find(f =>
                            f.Name.Equals(tvChannelDTO.Name, StringComparison.InvariantCultureIgnoreCase));
                        if (tvChannel != null) tvChannels.Remove(tvChannel);
                        tvChannels.Add(new TvChannel(tvChannelDTO.Name, tvChannelDTO.StartTime,
                            tvChannelDTO.StreamUrl));
                    }
            ...
            if (fixtureDto.CoverageInfo != null)
                CoverageInfo = new CoverageInfo(fixtureDto.CoverageInfo.Level, fixtureDto.CoverageInfo.IsLive,
                    fixtureDto.CoverageInfo.Includes, fixtureDto.CoverageInfo.CoveredFrom);
            ...
            if (fixtureDto.ReferenceIds != null) References = new Reference(new ReferenceIdCI(fixtureDto.ReferenceIds));

            if (fixtureDto.ScheduledStartTimeChanges != null)
                ScheduledStartTimeChanges =
                    fixtureDto.ScheduledStartTimeChanges.Select(s => new ScheduledStartTimeChange(s));
        }
        ...
        /// <summary>
        ///     Gets a <see cref="IReadOnlyDictionary{TKey,TValue}" /> containing additional information about the
        ...
        /// <summary>
        ///     Gets a <see cref="IProductInfo" /> instance providing sportradar related information about the sport event
        ///     associated
        ...
        {
            var info = ExtraInfo == null
                ? string.Empty
                : string.Join(", ", ExtraInfo.Keys.Select(k => $"{k}={ExtraInfo[k]}"));
            return
                $"StartTime={StartTime}, StartTimeConfirmed={StartTimeConfirmed}, StartTimeTBD={StartTimeTBD}, ReplacedBy={ReplacedBy}, ExtraInfo=[{info}]";
        }
        ...
        {
            var tv = TvChannels == null
                ? string.Empty
                : string.Join(", ", TvChannels.Select(k => ((TvChannel) k).ToString("f")));
            var res = PrintC();
            res +=
                $", TvChannels=[{tv}], CoverageInfo=[{((CoverageInfo) CoverageInfo)?.ToString("f")}], ProductInfo=[{((ProductInfo) ProductInfo)?.ToString("f")}]";
            return res;
        }
