...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.EntitiesImpl
{
    ...
    internal class Competitor : Player, ICompetitorV1
    {
            ...
            : base(ci.Id, new Dictionary<CultureInfo, string>())
        {
            ...
            {
                ReferenceIdCI q;
                if (competitorsReferences.TryGetValue(ci.Id, out q)) _referenceId = q;
            }
            else
            {
                if (ci.ReferenceId != null) _referenceId = ci.ReferenceId;
            }
        }

        /// <summary>
        ///     Gets a <see cref="IReadOnlyDictionary{CultureInfo, String}" /> containing competitor's country name in different
        ///     languages
        /// </summary>
        public IReadOnlyDictionary<CultureInfo, string> Countries => new ReadOnlyDictionary<CultureInfo, string>(
            _cultures.Where(c => GetCompetitor().GetCountry(c) != null)
                .ToDictionary(c => c, GetCompetitor().GetCountry));

        /// <summary>
        ///     Gets a <see cref="IReadOnlyDictionary{CultureInfo, String}" /> containing competitor's abbreviations in different
        ///     languages
        /// </summary>
        public IReadOnlyDictionary<CultureInfo, string> Abbreviations => new ReadOnlyDictionary<CultureInfo, string>(
            _cultures.Where(c => GetCompetitor().GetAbbreviation(c) != null)
                .ToDictionary(c => c, GetCompetitor().GetAbbreviation));

        /// <summary>
        ///     Gets a value indicating whether the current <see cref="ICompetitor" /> is virtual - i.e. competes in a virtual
        ///     sport
        /// </summary>
        ...
        public override IReadOnlyDictionary<CultureInfo, string> Names
        {
            get
            {
                ...
                lock (_lock)
                {
                    _names = _cultures.Where(c => _competitorCI.GetName(c) != null)
                        .ToDictionary(c => c, _competitorCI.GetName);
                    return _names;
                }
            }
        }
        ...
        {
            var abbreviations = string.Join(", ",
                Abbreviations.Select(x => x.Key.TwoLetterISOLanguageName + ":" + x.Value));
            var associatedPlayers = string.Empty;
            if (AssociatedPlayers != null && AssociatedPlayers.Any())
            {
                associatedPlayers = string.Join(", ",
                    AssociatedPlayers.Select(s => s.Id + ": " + s.GetName(_cultures.First())));
            ...
            var reference = _referenceId?.ReferenceIds == null || !_referenceId.ReferenceIds.Any()
                ? string.Empty
                : _referenceId.ReferenceIds
                    .Aggregate(string.Empty, (current, item) => current = $"{current}, {item.Key}={item.Value}")
                    .Substring(2);
            return
                $"{base.PrintC()}, Gender={Gender}, Reference={reference}, Abbreviations=[{abbreviations}]{associatedPlayers}";
        }
        ...
        protected override string PrintF()
        {
            ...
            return
                $"{base.PrintF()}, Countries=[{countryNames}], Reference={reference}, Abbreviations=[{abbreviations}], IsVirtual={IsVirtual}{associatedPlayers}";
        }
            ...
            lock (_lock)
            {
                if (_competitionCI != null || _competitorCI != null)
                {
                    var task = Task.Run(async () =>
                    {
                        ...
                        {
                            ReferenceIdCI q;
                            if (competitorsReferences.TryGetValue(_competitorCI.Id, out q)) _referenceId = q;
                        }
                        else
                        {
                            if (GetCompetitor().ReferenceId != null) _referenceId = GetCompetitor().ReferenceId;
                        }
                    });
                    task.Wait();
                }
        ...
        protected void FetchEventCompetitorsQualifiers()
        {
            lock (_lock)
            {
                if (_competitionCI != null)
                {
                    var task = Task.Run(async () =>
                    {
                        var competitorsQualifiers =
                            await _competitionCI.GetCompetitorsQualifiersAsync().ConfigureAwait(false);
